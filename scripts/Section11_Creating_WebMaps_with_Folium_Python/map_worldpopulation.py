...
for name,lat,lon,elev in zip(vol_names,vol_lat,vol_lon,vol_elev):
    ...
    fg.add_child(folium.CircleMarker(location=[lat,lon], radius=6, popup=folium.Popup(name,parse_html=True), fill="true", fill_color=color_producer(elev), color = 'grey'))  # this is for using Circle Marker
    print(name,lat,lon)
